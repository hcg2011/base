...
import com.chungo.base.http.progress.body.ProgressRequestBody
import com.chungo.base.http.progress.body.ProgressResponseBody
...
class ProgressManager private constructor() {
     ...
     */
    private val mRequestListeners = WeakHashMap<String, MutableList<ProgressListener>>()
    private val mResponseListeners = WeakHashMap<String, MutableList<ProgressListener>>()
    private val mHandler: Handler //所有监听器在 Handler 中被执行,所以可以保证所有监听器在主线程中被执行
    ...
    fun addRequestListener(url: String, listener: ProgressListener) {
        ...
        synchronized(ProgressManager::class.java) {
            ...
            if (progressListeners == null) {
                ...
                mRequestListeners[url] = progressListeners!!
            }
        }
    ...
    fun addResponseListener(url: String, listener: ProgressListener) {
        ...
        synchronized(ProgressManager::class.java) {
            ...
            if (progressListeners == null) {
                ...
                mResponseListeners[url] = progressListeners!!
            }
        }
        progressListeners!!.add(listener)
    }
    ...
    fun wrapRequestBody(request: Request?): Request? {
        ...
        if (mRequestListeners.containsKey(key)) {
            val listeners = mRequestListeners[key]
            return request.newBuilder()
                    .method(request.method(), ProgressRequestBody(mHandler, request.body()!!, listeners!!, mRefreshTime))
                    .build()
        }
        return request
    }
    ...
    fun wrapResponseBody(response: Response?): Response? {
        ...
        if (mResponseListeners.containsKey(key)) {
            val listeners = mResponseListeners[key]
    ...
    fun addDiffResponseListenerOnSameUrl(originUrl: String, listener: ProgressListener): String {
        return addDiffResponseListenerOnSameUrl(originUrl, (SystemClock.elapsedRealtime() + listener.hashCode()).toString(), listener)
    }
     ...
     */
    fun addDiffResponseListenerOnSameUrl(originUrl: String, key: String, listener: ProgressListener): String {
    ...
    fun addDiffRequestListenerOnSameUrl(originUrl: String, listener: ProgressListener): String {
        return addDiffRequestListenerOnSameUrl(originUrl, (SystemClock.elapsedRealtime() + listener.hashCode()).toString(), listener)
    }
     ...
     * @return `originUrl` 结合 `key` 生成的新的 `url`
     */
    fun addDiffRequestListenerOnSameUrl(originUrl: String, key: String, listener: ProgressListener): String {
        val newUrl = originUrl + IDENTIFICATION_NUMBER + key
     ...
     * @return
     */
    private fun haveRedirect(response: Response): Boolean {
        val status = response.code().toString()
        if (TextUtils.isEmpty(status))
            return false
        return if (status.contains("301") || status.contains("302") || status.contains("303") || status.contains("307")) {
            true
        } else false
    }
     ...
     * @param url      `url` 地址
     */
    private fun resolveRedirect(map: MutableMap<String, MutableList<ProgressListener>>, response: Response, url: String): String? {
    ...
    private fun forEachListenersOnError(map: Map<String, List<ProgressListener>>, url: String, e: Exception) {
        if (map.containsKey(url)) {
            val progressListeners = map[url]
